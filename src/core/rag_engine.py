...
2. **استخدم المعلومات من النص فقط - ممنوع تماماً اختلاق أي معلومة غير موجودة في النص.**
   ...
   - **ممنوع تماماً** ذكر أي محافظة أو منطقة غير موجودة في النص (مثل: البصرة، الموصل، النجف، الكوفة، كربلاء، الناصرية، ميسان، كركوك، سليمانية، أربيل، دهوك، أو أي محافظة أخرى)
   - **المناطق الصحيحة الوحيدة** المذكورة في النص: بغداد، ديالى، بابل (بما في ذلك المسيب، الإسكندرية، سدة الهندية) - **لا توجد محافظات أخرى في البيانات**
...
4. **عند السؤال عن الباقات والأسعار:**
   - **الباقات الصحيحة الوحيدة في البيانات:**
     * **باقات الفايبر (FTTH)**: فايبر 35، فايبر 50، فايبر 75، فايبر 150 فقط
     * **باقات الوايرلس (WiFi)**: Star، Sun، Neptune، Galaxy Star فقط
     * **عروض المنصة الترفيهية**: شهر واحد، ثلاثة أشهر، ستة أشهر فقط
   - **ممنوع تماماً اختلاق باقات غير موجودة مثل:**
     * "باقة سريع 4G" أو "باقة سريع 3G" - هذه غير موجودة
     * "باقة واي فاي" - الباقات الصحيحة هي Star، Sun، Neptune، Galaxy Star
     * "باقة سحابة" أو "Cloud" - هذه غير موجودة
     * أي باقة أخرى غير المذكورة أعلاه
   ...
   - **عند السؤال عن "جميع الباقات" أو "ما هي الباقات" أو "ما هي جميع باقات الإنترنت":**
     * **يجب ذكر كل باقة موجودة في النص فقط - لا تختلق أي باقة:**
       - باقات الفايبر (FTTH): فايبر 35 (35,000 دينار)، فايبر 50 (45,000 دينار)، فايبر 75 (65,000 دينار)، فايبر 150 (100,000 دينار)
       - باقات الوايرلس (WiFi): Star (30,000 دينار)، Sun (40,000 دينار)، Neptune (65,000 دينار)، Galaxy Star (100,000 دينار)
       - عروض المنصة الترفيهية: شهر واحد (12,000 دينار)، ثلاثة أشهر (25,000 دينار)، ستة أشهر (45,000 دينار)
     * ابحث في **جميع أقسام النص** عن الباقات
     * لا تتجاهل أي باقة حتى لو كانت في فقرة مختلفة
     * اذكر كل باقة مع اسمها وسعرها الدقيق
     * **ممنوع تماماً** ذكر أي باقة غير موجودة في النص
   - **اجمع كل الباقات** من جميع الأقسام في النص
   - اذكر **كل باقة موجودة فقط** مع اسمها وسعرها الدقيق
...
5. **عند السؤال عن مناطق التغطية:**
   - **المناطق الصحيحة الوحيدة في البيانات**: بغداد، ديالى، بابل (بما في ذلك المسيب، الإسكندرية، سدة الهندية)
   - **ممنوع تماماً** ذكر أي محافظة أو منطقة أخرى غير موجودة في النص مثل: البصرة، الموصل، النجف، الكوفة، كربلاء، الناصرية، ميسان، كركوك، سليمانية، أربيل، دهوك
   - **يجب أن تذكر فقط**: بغداد، ديالى، بابل (والمسيب، الإسكندرية، سدة الهندية)
   - إذا لم توجد معلومات عن منطقة معينة في النص، لا تذكرها أبداً
   - **تحذير صارم**: إذا ذكرت أي منطقة غير بغداد، ديالى، أو بابل، فأنت تختلق معلومات غير موجودة في البيانات
...
8. **ممنوع تماماً:**
   ...
   - **اختلاق باقات غير موجودة** مثل: "باقة سريع 4G"، "باقة سريع 3G"، "باقة واي فاي"، "باقة سحابة"، "Cloud"، أو أي باقة أخرى غير: فايبر 35/50/75/150، Star/Sun/Neptune/Galaxy Star، وعروض المنصة
   - **الباقات الصحيحة الوحيدة**: فايبر 35، فايبر 50، فايبر 75، فايبر 150، Star، Sun، Neptune، Galaxy Star، وعروض المنصة (شهر واحد، ثلاثة أشهر، ستة أشهر)
   - ذكر محافظات أو مناطق غير موجودة في النص (البصرة، الموصل، النجف، الكوفة، كربلاء، الناصرية، ميسان، كركوك، سليمانية، أربيل، دهوك، أو أي محافظة أخرى غير بغداد، ديالى، بابل)
   - **خاصة عند السؤال عن التغطية**: يجب أن تذكر فقط بغداد، ديالى، بابل (والمسيب، الإسكندرية، سدة الهندية) - ممنوع تماماً ذكر أي محافظة أخرى
...
def filter_and_deduplicate_docs(docs, question: str = "") -> str:
    ...
    invalid_regions = ["كوفة", "النجف", "بصرة", "البصرة", "الناصرية", "ميسان", "كربلاء", "كركوك", "سليمانية", "أربيل", "دهوك", "موصل", "الموصل", "نينوى"]
...
def validate_answer(answer: str, context: str) -> tuple[bool, str]:
    ...
    invalid_regions = ["كوفة", "النجف", "بصرة", "البصرة", "الناصرية", "ميسان", "كربلاء", "كركوك", "سليمانية", "أربيل", "دهوك", "موصل", "الموصل", "نينوى", "نينوى"]
    valid_regions = ["بغداد", "ديالى", "بابل", "المسيب", "الإسكندرية", "سدة الهندية"]
    ...
    suspicious_patterns = [
        ...
        # باقات مختلقة شائعة
        "سريع 4g", "سريع 3g", "باقة سريع 4g", "باقة سريع 3g",
        "4g", "3g", "باقة 4g", "باقة 3g",
        "باقة واي فاي", "واي فاي", "wifi package", "wifi باقة",
        "باقة سحابة", "سحابة", "cloud", "باقة cloud",
        "سحابة شخصي", "سحابة تجاري", "سحابة مؤسسي",
        "cloud personal", "cloud commercial", "cloud enterprise"
    ]
...
def get_answer(question: str, previous_question: str = None, previous_answer: str = None, max_retries: int = 2) -> str:
    ...
    try:
        ...
        # فحص إضافي صارم: إذا كان السؤال عن التغطية، تأكد من عدم وجود مناطق غير صحيحة
        if any(w in clean_q.lower() for w in ["تغطية", "منطقة", "محافظة", "أين", "فرع"]):
            invalid_regions_check = ["البصرة", "بصرة", "الموصل", "موصل", "النجف", "نجف", "الكوفة", "كوفة", 
                                     "كربلاء", "الناصرية", "ناصرية", "ميسان", "كركوك", "سليمانية", "أربيل", "دهوك", "نينوى"]
            response_lower = response.lower()
            for invalid_region in invalid_regions_check:
                if invalid_region in response_lower and invalid_region not in context_used.lower():
                    logger.error(f"❌ تم اكتشاف منطقة غير صحيحة في الإجابة النهائية: {invalid_region}")
                    # إذا اكتُشفت منطقة غير صحيحة، استخدم إجابة آمنة مباشرة
                    response = "نغطي بغداد (جميع الأحياء والمناطق)، ديالى (جميع المناطق)، وبابل (بما في ذلك المسيب، الإسكندرية، سدة الهندية). نسعى لتوسيع التغطية في جميع المحافظات بالتعاون مع المشروع الوطني للإنترنت."
                    break
        
        # فحص إضافي صارم: إذا كان السؤال عن الباقات، تأكد من عدم وجود باقات مختلقة
        if any(w in clean_q.lower() for w in ["باقة", "باقات", "جميع", "ما هي", "قائمة"]):
            invalid_packages_check = [
                "سريع 4g", "سريع 3g", "باقة سريع 4g", "باقة سريع 3g",
                "4g", "3g", "باقة 4g", "باقة 3g",
                "باقة واي فاي", "واي فاي باقة", "wifi package",
                "باقة سحابة", "سحابة", "cloud", "باقة cloud",
                "سحابة شخصي", "سحابة تجاري", "سحابة مؤسسي"
            ]
            response_lower = response.lower()
            for invalid_package in invalid_packages_check:
                if invalid_package in response_lower and invalid_package not in context_used.lower():
                    logger.error(f"❌ تم اكتشاف باقة مختلقة في الإجابة النهائية: {invalid_package}")
                    # إذا اكتُشفت باقة مختلقة، استخدم إجابة آمنة مباشرة
                    response = """الباقات المتوفرة:

باقات الفايبر (FTTH):
- فايبر 35: 35,000 دينار عراقي شهرياً - سرعة 35 Mbps
- فايبر 50: 45,000 دينار عراقي شهرياً - سرعة 50 Mbps
- فايبر 75: 65,000 دينار عراقي شهرياً - سرعة 75 Mbps
- فايبر 150: 100,000 دينار عراقي شهرياً - سرعة 150 Mbps

باقات الوايرلس (WiFi):
- Star: 30,000 دينار عراقي شهرياً
- Sun: 40,000 دينار عراقي شهرياً
- Neptune: 65,000 دينار عراقي شهرياً
- Galaxy Star: 100,000 دينار عراقي شهرياً

عروض المنصة الترفيهية:
- شهر واحد: 12,000 دينار عراقي
- ثلاثة أشهر: 25,000 دينار عراقي
- ستة أشهر: 45,000 دينار عراقي"""
                    break
